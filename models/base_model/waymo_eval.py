...
def transform_preds_to_waymo_format(pred_dicts, top_k_for_eval=-1, eval_second=8):
    ...
    for scene_idx, val in enumerate(scene2preds.items()):
        ...
        for obj_idx, cur_pred in enumerate(preds_per_scene):
            ...
            gt_trajs[scene_idx, obj_idx] = cur_pred['gt_trajs'][:num_frames_in_total, [0, 1, 3, 4, 6, 7,
                                                                                       8]]  # (num_timestamps_in_total, 10), [cx, cy, cz, dx, dy, dz, heading, vel_x, vel_y, valid]
...
def waymo_evaluation(pred_dicts, top_k=-1, eval_second=8, num_modes_for_eval=6):
    ...
    for cur_metric_name in ['minADE', 'minFDE', 'MissRate', 'mAP']:
        ...
        for cur_name in ['VEHICLE', 'PEDESTRIAN', 'CYCLIST']:
            ...
            result_format_list[name_to_row[cur_name]][name_to_col[cur_metric_name]] = '%.4f,' % avg_results[
    ...
    result_dict[
        '-----Note that this evaluation may have marginal differences with the official Waymo evaluation server-----'] = 0
