...
class MTRDecoder(nn.Module):
    ...
    def apply_dense_future_prediction(self, obj_feature, obj_mask, obj_pos):
        ...
        obj_pos_valid = obj_pos[obj_mask][..., 0:2] #滤除z坐标 [64,2]
        obj_feature_valid = obj_feature[obj_mask] #提取有效特征  [64,512]
        obj_pos_feature_valid = self.obj_pos_encoding_layer(obj_pos_valid)  #[64,512]
        obj_fused_feature_valid = torch.cat((obj_pos_feature_valid, obj_feature_valid), dim=-1) #拼接位置和特征编码[64,1024]

        pred_dense_trajs_valid = self.dense_future_head(obj_fused_feature_valid) #预测模块 [64,60*7]
        pred_dense_trajs_valid = pred_dense_trajs_valid.view(pred_dense_trajs_valid.shape[0], self.num_future_frames, 7) #[64,60,7]
        #将预测的轨迹位置 调整为当前目标位置的全局坐标
        temp_center = pred_dense_trajs_valid[:, :, 0:2] + obj_pos_valid[:, None, 0:2] # [64,60,2]
        pred_dense_trajs_valid = torch.cat((temp_center, pred_dense_trajs_valid[:, :, 2:]), dim=-1)# [64,60,7]

        # future feature encoding and fuse to past obj_feature 提取未来特征(位置和速度)并编码
        obj_future_input_valid = pred_dense_trajs_valid[:, :, [0, 1, -2, -1]].flatten(start_dim=1,
                                                                                      end_dim=2)  # (num_valid_objects, C)
        obj_future_feature_valid = self.future_traj_mlps(obj_future_input_valid) #[64,240]
        #融合历史与未来特征 并更新
        ...
        #特征回填 无后验
        ret_obj_feature = torch.zeros_like(obj_feature)#[1,64,512]
        ret_obj_feature[obj_mask] = obj_feature_valid #[64,512]
        #有后验
        ret_pred_dense_future_trajs = obj_feature.new_zeros(num_center_objects, num_objects, self.num_future_frames, 7)# [1,64,60,7]
        ret_pred_dense_future_trajs[obj_mask] = pred_dense_trajs_valid #[64,60,7]
        self.forward_ret_dict['pred_dense_trajs'] = ret_pred_dense_future_trajs #[64,60,7] 将预测结果存入字典
    ...
    def forward(self, batch_dict):
        ...
        center_objects_feature = batch_dict['center_objects_feature'] #[1,512]
        num_center_objects, num_objects, _ = obj_feature.shape
