...
def side_to_directed_lineseg(
    ...
    输入参数维度:
        query_point (np.ndarray): 待判断点的坐标 [2] (x,y)
        start_point (np.ndarray): 线段起点坐标 [2] (x,y)
        end_point (np.ndarray): 线段终点坐标 [2] (x,y)
        
    返回:
        str: 位置关系 ('LEFT'/'RIGHT'/'CENTER')
        
    实现原理:
        通过计算向量叉积判断点的方位：
        - 将线段视为从start_point到end_point的向量
        - 计算(end - start)与(query - start)的叉积
        - 叉积>0: 点在线段左侧
        - 叉积<0: 点在线段右侧
        - 叉积=0: 点在线段延长线上
    """
